...
const Header = ({ showModal }) => {
	...
	return (
		<Affix className="bg-[#fff] opacity-100 z-50" offsetTop={0}>
			<Row
				align={'middle'}
				className={`${shouldChangeBackground ? 'bg-[#ecebeb] opacity-90' : ''} p-5 text-black`}
				gutter={16}
			>
				<Col span={8}>
					...
					<Row gutter={4}>
						<Col lg={4} span={6}>
							<Link
								...
								className={
									isMenu === ''
										? `bg-[#FFB07F] rounded-lg p-3 md:text-xl`
										: 'hover:bg-[#FFB07F] rounded-lg p-3 md:text-xl'
								}
							>
								Home
							</Link>
				...
				<Col className="hidden md:block" span={2}>
					<Row>
						<Col span={12}>
							<SearchOutlined onClick={showModal} className="text-xl rounded-lg p-1" />
						</Col>
						<Col span={12}>
							<Tooltip
									...
									<div className="px-3">
										{!cookies ? (
											<>
												...
												<p onClick={() => null} className="text-black cursor-pointer">
													Logout
												</p>
											</>
										) : (
							...
							>
								<UserOutlined
									className={
										isMenu === 'profile'
											? 'bg-red-400 rounded-lg p-1 text-xl cursor-pointer'
											: 'text-xl hover:bg-red-400 rounded-lg p-1 cursor-pointer'
									}
								/>
							</Tooltip>
						</Col>
