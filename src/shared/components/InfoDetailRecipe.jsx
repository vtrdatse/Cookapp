...
const InfoDetailRecipe = ({ recipe }) => {
    const [value, setValue] = useState(Math.round(recipe.rate));
                    ...
                    <Col span={24}>
                        <Row className="mt-3" align={"middle"} justify="center" gutter={8}>
                            {recipe && recipe.image.slice(1, recipe.image.length).map((item, index) => {
                                return (
                                    <Col key={index} span={24 / recipe.image.length}>
                                        <div
                ...
                <Row className='w-full text-black' gutter={16}>
                    ...
                    <Col span={24}>
                        <Row align="middle" justify={"start"}>
                            <Col span={8}>
                                <span>
                                    <Rate tooltips={tooltipArray} value={value} />
                                    {value ? <span className="ant-rate-text">{tooltipArray[value - 1]}</span> : ''}
                                </span>
                            </Col>
                    ...
                    <Col span={24}>
                        <div className="flex justify-start items-center gap-3 mt-3">
                            ...
                            <Button onClick={()=> {cookies ? navigate("/login") : ""}} type="button" icon={<StarOutlined className="p-0 m-0 align-middle" />}>Rating</Button>
